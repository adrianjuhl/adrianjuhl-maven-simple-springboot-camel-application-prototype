...
# Build, install, and generate site (mvn clean install site), with version update and dependency-check reports.
...
mvn_clean_install_site()
{
  mvn \
      ...
      install \
      site \
      -Pdisplay-version-updates \
      -Preport-version-updates \
      -Pdisplay-dependency-check \
      -Preport-dependency-check
}
